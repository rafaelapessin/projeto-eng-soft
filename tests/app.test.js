...
describe('Testes da aplicação', () => {
  ...
  test('Deve criar uma nova reserva com sucesso', async () => {
    const res = await request(app).post('/reservar').send({
      sala: 'Laboratório de Informática', dia: '2025-10-10', aula: '1', turma: '2ºIPI01', professor: 'João', disciplina: 'Matemática'
    })
  ...
  test('Deve detectar conflito de reserva', async () => {
    await request(app).post('/reservar').send({
      sala: 'Laboratório de Informática', dia: '2025-10-10', aula: '1', turma: '3ºMSI01', professor: 'João', disciplina: 'Matemática'
    })
    const res = await request(app).post('/reservar').send({
      sala: 'Laboratório de Informática', dia: '2025-10-10', aula: '1', turma: '2ºIPI02', professor: 'Maria', disciplina: 'História'
    })
